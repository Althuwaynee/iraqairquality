...
output_folder = 'IRQ/monthly_plots'  # Folder to save monthly plots
...
# Add year, month and day-of-month
all_data['year'] = all_data['datetime'].dt.year
all_data['month'] = all_data['datetime'].dt.month
all_data['day_of_month'] = all_data['datetime'].dt.day

# Compute daily median
print("Calculating daily medians...")
groups = all_data.groupby(['year', 'month', 'day_of_month'])['dust_concentration_ugm3']
...
for month in range(1, 13):
    print(f"Creating plot for {month_names[month-1]}...")
    
    # Filter data for this month
    month_data = daily_median[daily_median['month'] == month]
    
    if month_data.empty:
        print(f"No data found for {month_names[month-1]}")
        continue
    ...
    for i, year in enumerate(years):
        ...
        ax.plot(
            year_data['day_of_month'], year_data['dust_concentration_ugm3'],
            color=colors[i], linestyle='-', linewidth=1.1,
            marker='o',markersize=2, label=str(year)
        )
    
    # Labels and ticks
    plt.title(f'Daily (Median 3-hour steps) Dust Concentration in IRAQ - {month_names[month-1]}', fontsize=14, pad=10)
    ...
    # Create legend
    year_handles = [plt.Line2D([], [], color=colors[i], marker='o', linestyle='-', markersize=3, label=str(year))
                    for i, year in enumerate(years)]
    ...
    # Save and close
    plt.tight_layout()
    output_plot = os.path.join(output_folder, f'dust_concentration_timeseries_{month_names[month-1]}.png')
    plt.savefig(output_plot, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Plot saved as {output_plot}")

print("All monthly plots created successfully.")
