...
def handle_current(chat_id):
    ...
    if lang == "ar":
        message = (
            ...
            f"🏭 الغبار العالق: <b>{int(pm10)} µg/m³</b>\n"
            f"📈 المستوى: <b>{level}</b>\n\n"
            f"{level_desc}\n\n"
...
def handle_help(chat_id):
    ...
    help_text = (
        ...
        "ابدا - لمشاركة الموقع وتفعيل الاشعارات\n"
        "الان - لفحص جودة الهواء الآن\n"
        "وضعي - للتحقق من حالة اشتراكك\n"
        "توقف - لإيقاف استلام التنبيهات\n"
        "استئناف - لاستئناف استلام التنبيهات\n"
        "لغة - لتغيير اللغة  (AR/EN)\n"
        "مساعدة - لعرض قائمة الاوامر الحالية المعروضة امامك\n\n"
        "📍 التطبيق سوف يرسل لك رسالة تنبهك بتغير جودة الهواء حولك, طاب يومكم."
    )
